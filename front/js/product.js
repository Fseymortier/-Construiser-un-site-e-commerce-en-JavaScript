...
function createCanapeSelected(canape) {
    ...
    button.addEventListener('click', () => {
        ...
        if (canapeQuantity < 1) {
            alert(
                'Vous pouvez ajouter un minimum de 1 canapé par type et par couleur'
            );
            isCanapeFound = true;
        }
        if (canape.quantity > 100) {
            alert(
                'Vous pouvez ajouter un maximum de 100 canapés par type et par couleur'
            );
            isCanapeFound = true;
        }
        var arrayStorage = [];
        ...
        for (var i = 0; i < arrayStorage.length; i++) {
            let quantityTotal =
                parseInt(arrayStorage[i].quantity) + parseInt(canape.quantity);
            if (
                canape._id == arrayStorage[i]._id &&
                canape.colorChoice == arrayStorage[i].colorChoice
            ) {
                if (quantityTotal > 100) {
                    alert(
                        'Vous avez déja ' +
                            arrayStorage[i].quantity +
                            ' canapés de cette couleur dans votre panier. Vous pouvez ajouter un maximum de 100 canapés par type et par couleur'
                    );
                    isCanapeFound = true;
                } else {
